...
def main(args):
        ...
        # attacked_ids = [0, 1, 2, 3]
        attacked_ids = list(range(101))
        # attacked_ids = [0, 1, 2, 3]
            
        ### tt_test
        # loading cfg
        cfg_path = CONFIG_PATHS[args.model_name]
        cfg = get_cfg_custom(cfg_path, args.batch_size)
        cfg.CONFIG.MODEL.PRETRAINED = False

        # loading model.
        ckpt_path = MODEL_TO_CKPTS[args.model_name]
        vid_model = get_model(cfg)
        vid_model.load_state_dict(torch.load(ckpt_path)['state_dict'])
        vid_model.cuda()
        vid_model.eval()
        ...
        def GetPairs_ori_tt_ucf_test( idx):
            x0 = torch.from_numpy(np.load('/home/pangbo/reproduction/TT-master/output/curve_nonlocal101_ucf101/UCF-i3d_resnet101-Group_Meta-False-10-20_4/{}-ori.npy'.format(idx)))
            # x0 = image_to_vector(model_name, dataset_name, x0)  # 将视频归一化在[0,1]之间
